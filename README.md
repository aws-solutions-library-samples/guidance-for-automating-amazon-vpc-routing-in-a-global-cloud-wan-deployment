...
AWS Cloud WAN is a service that you can use to build, manage, and monitor a unified global network that connects resources running across your cloud and on-premises environments. Cloud WAN ensures dynamic route propagation across multiple regions. So for a sample deployment that spans across 2 regions, and where each region has 3 VPCs: you'll create VPC attachments into Cloud WAN segments. Cloud WAN will ensure that each VPC's CIDR is dynamically learned across regions if you propagate each VPC's CIDR into Cloud WAN's segment routing table. To ensure that end to end connectivity works between your VPCs, you'll need to update the routing tables of the VPCs to provide network connectivity for workloads that reside within the VPCs. This is illustrated in Figure-XYZ below:

<< INSERT DIAGRAM >>

In this Guidance, we'll see how to automatically push routes to VPC routing tables. This will automate the task of updating each VPC's routing table, and provide you with a fully automated end-to-end routing solution wherein workloads residing within VPCs have network connectivity to each other irrespective of the region where a VPC resides.
...
### Scenario and Solution's details

You have a global network, across 2 regions. Cloud WAN provides global network orchestration across these regions, and there are 3 Cloud WAN segments, one each for departments named Finance, Sales and HR. VPC IPAM stores the CIDRs assigned to each of the 3 departments. Note that this solution uses VPC IPAM purely to store the CIDRs of each department. You can optionally use VPC IPAM to provide CIDRs for your VPCs.

![Cloud WAN Infrastructure](./img/cloud-wan-demo.png)

Whenever a VPC's attachment to Cloud WAN is created. The event has a JSON format and contains important information about the details of the VPC attachment. A sample event is shown in Figure-ABC below:

<< Insert diagram of an event's snapshot >>

The solution in this Guidance uses Event Bridge and Lambda to detect the 'attachment created' event generated by Cloud WAN. EventBridge triggers a regional lambda function. The lambda function has the necessary code and logic that fetches the department specific CIDR from VPC IPAM. After getting the department CIDR, the Lambda function creates a Prefix List, and finally pushes a route into the VPC's routing table. The destination for this route is the Prefix List and the target for the route is the Cloud WAN core network.
...
Note that the instructions provided in this section build out all the necessary Cloud WAN and IPAM constructs that are shown in Figure-XYZ. Please navigate to "Using this solution for an existing deployment" section in case you'd like to use this solution for an existing deployment wherein you already have a Cloud WAN network or your own IP addressing scheme.
...
1. Delete CloudFormation stacks. This will remove all the stacks. (cloud wan, ipam, regional infrastructure, eventbridge rules, and Cloud WAN VPC attachments). Note that this operation must be done from within the samples directory that you navigated to in Step 1.
...
### Instructions for an existing/customized deployment

This section details out the instructions you'll need to use this solution with an existing deployment, or in case you'd like to customize your Cloud WAN or IPAM deployment according to your use case.
...
This solution uses VPC IPAM to store the CIDRs that are needed for populating VPC's routing tables. To use this solution for an exsiting or customized deployment, the first step is to create VPC IPAM Pools. Check out VPC IPAM documentation << Insert Link >> for details on how to create VPC IPAM pools.

Make a note of the VPC IPAM Pools' name. You'll use the same name as tag while creating VPC attachments into Cloud WAN. 

The snapshot below shows a VPC IPAM pool named 'finance' that has a CIDR of 172.16.0.0/14

#### Step-2: Create Cloud WAN deployment

Create your Cloud WAN deployment and the corresponding Cloud WAN policy based on your use-case. Check out Cloud WAN documentation << Insert link >> for details.
...
Repeat steps 3.1 and 3.2 for each of your target regions. For example, if you want to deploy in us-west-1 and us-east-1, you would repeat steps 3.1 and 3.2 once each for us-west-1 and us-east-1.

##### Step-3.1: Create regional stack

Use regional_lambda.yaml and deploy this cloudformation template in the target region. This stack deploys a regional Event Bridge rule that triggers a regional Lambda function. The Lambda function includes the necessary code for fetching the correct CIDR from IPAM, and pushing a route into VPC's routing table.
...
Deploy the eventbridge_rule.yaml in us-west-2 Region. This stack creates the Event Bridge rule that traps on 'VPC attachment created' and 'VPC attachment deleted' events. Cloud WAN generates events in us-west-2 which will be sent to the event bridge rule that is created by the cfn stack.
...
The value of the tag must match the name of the corresponding VPC IPAM pool.

For example: if you create a VPC IPAM pool named 'finance', ensure that you tag the VPC attachment with a tag key-value pair of:
department:finance
